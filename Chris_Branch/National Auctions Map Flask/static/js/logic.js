// d3.json("/domain/projects2").then(function (national_data) {
// console.log(national_data)
//  })

 queue().defer(d3.json, "/domain/projects2")
    .await(makeGraphs);


function makeGraphs(error, national_data) {
  console.log(national_data)
//   const location = [national_data['location/0'], national_data['location/1']]
// }
...
for (var i = 0; i < national_data.length; i++) {
  console.log(national_data[i]['location/0'])
  // Setting the marker radius for the state by passing population into the markerSize function
  auctionMarkers.push(
    L.circle([national_data[i]['location/0'],national_data[i]['location/1']], {
      stroke: false,
      fillOpacity: 0.75,
      color: "white",
      fillColor: "red",
      radius: national_data[i].numberListedForAuction*200
    })
  ...
  auction_salesMarkers.push(
    L.circle([national_data[i]['location/0'],national_data[i]['location/1']], {
      ...
      color: "purple",
      fillColor: "orange",
      radius: national_data[i].numberSold*200
    })
    .bindPopup("<h3> Location: " + national_data[i].cityname +
    "</h3><hr><p> Date: " + new Date(national_data[i].lastModifiedDateTime) + "</p>" + 
